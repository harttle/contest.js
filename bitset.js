class BitSet {
    ...
    count() {
        let count= 0
        for (let bit = 1n; bit <= this.val; bit <<= 1n) if (bit & this.val) count++
        return count
    }
    set(i, val) {
        if (val) this.val |= (1n << BigInt(i))
        else this.val &= ~(1n << BigInt(i))
    }
    get(i) {
        return this.val & 1n << BigInt(i) ? 1 : 0
    }
    toString() {
        if (this.N <= 0) return ''
        ...
        let ans = ''
        let last = null
        for (let val = this.val; val && val != last; last = val, val >>= 1n)
            ans = (val & 1n) + ans
